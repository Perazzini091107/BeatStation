...
if (isset($_GET['user'])) {
    $user = $conn->real_escape_string($_GET['user']); // Evita SQL Injection
} else {
...
$sql_posts = "SELECT idpost, user, content, caminho, timestamp FROM posts WHERE user = '$user' AND main_category = 'musica' ORDER BY timestamp DESC";
